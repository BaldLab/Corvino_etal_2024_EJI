...
### Output metadata
```{r output_metadata}
#output.df<- seurat.combined@meta.data %>%
#  dplyr::select(nCount_RNA, nFeature_RNA, group, 
#                condition, percent.mito, seurat_clusters,
#                UMAP_1, UMAP_2, PCA_1, PCA_2, condition_clust) 

#output.df$barcode <- rownames(output.df)

#output.df$barcode <- gsub(".*_", "", output.df$barcode)


#write.table(output.df, "metadata.txt", quote = FALSE, sep = "\t", row.names = F, col.names = T)
...
sort(unique(Cillo_et_al@meta.data$cd8_cells_clusterID))
