...
## Type I IFN cluster signature {.tabset}
...
### defining Type I signature
```{r defining_Type_I_signature}

##########################################
# Average dataset for visualisation
##########################################
...
# Predict cell state using signature
seurat.combined <- Predict.IFN(seurat.combined, 
                               gene.signature = IFN.sig,
                               sig.name = "IFNsig")

evaluate.prediction(seurat.combined, 
                    sig.name = "IFNsig", 
                    validation.cluster = "Type_I_IFN", 
                    validation.column = "seurat_clusters")
...
########################################
# Type I up-regulated signature
########################################

# number of top DEGs to use 
n.markers <- 50

IFN.sig.UP <- 
  TypeI.markers %>% 
  dplyr::filter(p_val_adj < 0.05) %>%
  dplyr::top_n(n.markers, avg_log2FC) %>%
  rownames()



# Write signature to file 
write.csv(IFN.sig.UP, "output/tables/TypeI_up_regulated_signature.csv")

```



## Type I IFN cluster evaluation {.tabset}

### Evaluating signature on HNSCC dataset
```{r HNSCC_dataset_evaluation}
    

# Read in type I gene signature
IFN.sig.UP <- read.csv("output/tables/TypeI_up_regulated_signature.csv", row.names = 1)
IFN.sig.UP <- IFN.sig.UP$x    
...
x <- prop.table(table(seurat.combined@meta.data$seurat_clusters, 
      seurat.combined@meta.data$IFNsig_Prediction)) * 100
...
### Evaluating signature on Cillo dataset

```{r Cillo_dataset_evaluation}
...
x <- prop.table(table(Cillo_et_al@meta.data$cd8_cells_clusterID, 
      Cillo_et_al@meta.data$IFNsig_Prediction)) * 100
...
```
  ...
  ### testing differential cytotoxicity profiles
```{r}
