...
temp.list <- str_split(seurat.tcr.filt@meta.data$CTgene, "\\.")

# First and second elements of the list are V and J calls
seurat.tcr.filt@meta.data$TRAV <- sapply(temp.list, "[[", 1)
seurat.tcr.filt@meta.data$TRAJ <- sapply(temp.list, "[[", 2)
...
seurat.tcr.filt@meta.data$TRAC <- sapply(temp.list.2, "[[", 1)
seurat.tcr.filt@meta.data$TRBV <- sapply(temp.list.2, "[[", 2)

# 4th and 5th elements of original list are TRBJ and TRBC calls respectively
seurat.tcr.filt@meta.data$TRBJ <- sapply(temp.list, "[[", 4)
seurat.tcr.filt@meta.data$TRBC <- sapply(temp.list, "[[", 6)
...
Idents(seurat.tcr.filt) <- seurat.tcr.filt@meta.data$Exhausted_Stim_potential_condition
...
DEG.df <- read.csv("output_tcr/figures/Exhausted_restimability/DEG_restimability_40_20.csv")


goi <- DEG.df %>% 
  filter(avg_log2FC > 1 | avg_log2FC < -0.75)
...
DoHeatmap(average.seurat, 
          #group.by = "Exhausted_Stim_potential_condition", 
          features = goi, 
          draw.lines = F)
...
top_n(Stim.potential.markers, -10, myAUC)
...
#DEG.df <- read.csv("output_tcr/figures/Clonotype_movement/DEG_restimability_40_20.csv")
...
#Nebulosa::plot_density(seurat.tcr.filt, feature = "MTRNR2L8", group.by = "Total_Stim_potential")



Idents(seurat.tcr.filt) <- seurat.tcr.filt@meta.data$Exhausted_Stim_potential

temp.seurat <- subset(seurat.tcr.filt, ident = c("Unstimable", "Stimable"))



Idents(temp.seurat) <- temp.seurat@meta.data$Exhausted_Stim_potential_condition

#temp.seurat <- subset(temp.seurat, ident = c("Unstimable_US", "Stimable_US"))

temp.seurat.small <- subset(temp.seurat, downsample = 100)
...
FeaturePlot(temp.seurat,
            feature = "MTRNR2L8",
            cols = c("Gray", "Red"),
            pt.size = 2,
            split.by = "Total_Stim_potential")
  ...
  # Get top 10 genes
  sig.genes <- DEG.df[DEG.df$p_val_adj < 0.05, ]
  
  # up-regulated genes
  genes.to.label.1 <- top_n(sig.genes, 5, avg_log2FC)$X
  
  # Down-regulated genes
  genes.to.label.2 <- top_n(sig.genes, -5, avg_log2FC)$X
  ...
  pattern.var <- c("TRA", "TRB")
logic.var <- grepl(paste0(pattern.var, collapse = "|"), genes.to.label)
genes.to.label <- genes.to.label[!logic.var]
  
  
  
  # Average expression
  avg.subset.seurat <- log1p(AverageExpression(temp.seurat, verbose = FALSE)$RNA)
  ...
  genes.to.label <- c("MBP", "GPR171", "GEM", "PGK1", "IL2RA")
  
  
  # Generate plot
  p1 <- ggplot(avg.subset.seurat, 
               aes(Unstimable_US, Stimable_US)) + 
    geom_point() + 
  ...
  #MTRNR2L8, ANXA1, JAML
  # GNLY, FTH1, MTRNR2L12, ACP5, CTSW, RPS4Y1, ZBED2, LTB, ITM2C, KIR2DL4
...
x <- table(seurat.tcr.filt@meta.data$TRAV, seurat.tcr.filt@meta.data$Stim_potential)
...
FeaturePlot(seurat.tcr.filt, 
            "MTRNR2L8")

FeaturePlot(seurat.tcr.filt, order = T,
            "CD38", pt.size = 2)
...
VlnPlot(seurat.tcr.filt, group.by = "seurat_clusters",
...
Stim.potential.markers <- FindMarkers(seurat.tcr.filt, 
...
x <- grep("^MTRNR.*", rownames(seurat.tcr.filt), value = T)
...
#library("Nebulosa")
#Nebulosa::plot_density(seurat.tcr.filt, "CD5")
...
# 
# seurat.tcr.filt@meta.data$highlight[!is.na(seurat.tcr.filt@meta.data$highlight)] <- "Unstimable"
# seurat.tcr.filt@meta.data$highlight[is.na(seurat.tcr.filt@meta.data$highlight)] <- "NA"
# 
# 
# print(DimPlot(seurat.tcr.filt, 
...
Query.clonotypes.US <- seurat.tcr.filt@meta.data %>%
  dplyr::filter(Clonotype_overlap == "Shared" & condition == "Stim") %>%
...
Query.clonotypes.stim <- seurat.tcr.filt@meta.data %>%
  dplyr::filter(Clonotype_overlap == "Shared" & condition == "Stim") %>%
  dplyr::filter(seurat_clusters %in% c("Stimulated_exhausted", "Exhausted_1", "Exhausted_2")) %>%
...
Clonotype.ids <- seurat.tcr.filt@meta.data %>%
...
cell.ids <- rep("Other", nrow(seurat.tcr.filt@meta.data))
names(cell.ids) <- rownames(seurat.tcr.filt@meta.data)
