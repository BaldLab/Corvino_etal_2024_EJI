...
# match only if an Alpha and paired beta are annotated to the same antigen 
vdjdb.alpha <- vdjdb %>% 
  filter(gene == "TRA")
...
for(i in seq_along(alpha.overlap)){
  
  temp <- vdj.overlap.alpha %>% 
  filter(cdr3 %in% alpha.overlap[i])
  
  
  #print(temp)
  
  
  temp2 <- vdj.overlap.beta %>% 
  filter(cdr3 %in% beta.overlap[i])
...
for(i in seq_along(CTaa.vec)){
  ...
  print(DimPlot(seurat.tcr, 
                ...
                split.by = "condition",
                cols = c("Red", "#BEBEBE33"),
                group.by = "highlight") + ggtitle(paste0(CTaa.vec[i])) + NoLegend()
  )
}
