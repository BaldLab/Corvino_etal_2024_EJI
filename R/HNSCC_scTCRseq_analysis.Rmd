...
pdf("Stimulated_TCRcircos_filt.pdf", height=10, width=10)
...
pdf(paste0("Untimulated_TCRcircos_filt.pdf"), height=10, width=10)
...
pdf(paste0("Untimulated_TCRcircos_filt_Proportion.pdf"), height=10, width=10)
chordDiagram(Chord.list3[["US"]], grid.col = grid.cols, self.link = 1, annotationTrack = "grid", 
    preAllocateTracks = list(track.height = max(strwidth(unlist(dimnames(Chord.list3[["US"]]))))))
circos.track(track.index = 1, panel.fun = function(x, y) {
    xlim = get.cell.meta.data("xlim")
    xplot = get.cell.meta.data("xplot")
    ylim = get.cell.meta.data("ylim")
...
color_regions  <- c(US = "turquoise", Stim = "red")
```
...
###Filtering out MAIT and gd Clusters
filt.meta <- subset(filt.meta, seurat_clusters != "gd_T_non_g9d2" & seurat_clusters != "gd_T_g9d2" & seurat_clusters != "MAIT")
...
###clonotypesOnly can be used to just count the clonotypes shared between cluster (removes cell numbers from the equation)
output <- getIntegratedCircle(filt.meta, proportion = FALSE, clonotypesOnly = FALSE)
...
####Ploting
pdf("TwoVariables_test_Circos1.pdf", height=10, width=10)

chordDiagram(df2, self.link = 1, annotationTrack = c("grid"),
    grid.col = grid.col,
    order = order,
