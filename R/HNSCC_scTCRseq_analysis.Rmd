...
remove.self.ribbons <- FALSE



# Remove non-alpha/beta T cell subsets from Circos plots
initial.n <- nrow(meta.tcr) # 6,625
...
filt.n <- nrow(meta.tcr) # 6,320
# therefore 305 cells filtered
initial.n - filt.n
...
filt.n-nrow(filt.meta) # 2,784 cells
rm(filt.n)
...
filt.meta %>% 
  dplyr::filter(condition == "US" & seurat_clusters == "Stimulated_1") %>%
  dplyr::summarise(n())
  
  CIVAQAGTALIF_CSVIDRAAGELFF
  
  filt.meta %>% 
  dplyr::filter(condition == "Stim" & CTaa == "CVVTSFESGNTPLVF_CASSQDLWDRNNEQFF") %>%
  dplyr::summarise(n())
  
  
 # dplyr::select(seurat_clusters) %>% unique()
...
# I dont think this is doing what is advertised
# Removing clonotypes with < 5 cells
#filt.meta <- 
  filt.meta %>%
...
###clonotypesOnly can be used to just count the clonotypes shared between cluster (removes cell numbers from the equation)
output <- getIntegratedCircle(filt.meta, 
                              proportion = TRUE, 
                              clonotypesOnly = TRUE)
...
# Filter out within condition ribbons 
df3 <- output[output$from_group != output$to_group, ]


df3 <- data.frame(from = paste(df3[[4]], df3[[1]], sep = "|"),
                 to = paste(df3[[5]], df3[[2]], sep = "|"),
                 value = df3[[3]],
                 stringsAsFactors = FALSE)
...
# Nick: Code below used to read data.frame(condition = c(output[[5]]))
# This takes the condition of the to_group column. 
# I think this is a typo and should be output[[4]] i.e the from_group column.
# Please advise if incorrect

combined <- data.frame(condition = c(output[[4]]), 
                       from = c(output[[1]]), 
                       to = c(output[[2]]), 
                       stringsAsFactors = FALSE)

combined <- combined[order(combined$condition, combined$from, combined$to), ]
order.val <- paste(combined$condition, combined$from, sep = "|")
grid.col <- structure(color_species[combined$from], names = order.val)
...
order.val <- rev(order.val)

order.val <- unique(df2$from)
order.val <- sort(order.val, decreasing = T)
