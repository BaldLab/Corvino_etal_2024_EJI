...
# )
# 
# for(i in seq_along(unique(combined$condition))) {
#   region <- unique(combined$condition)[i]
#   l <- combined$condition == region
#   sn <- paste(combined$condition[l], combined$from[l], sep = "|")
#   sn <- sn[sn != "US|Stimulated_exhausted" & sn != "US|Stimulated_1"] #These get accidentally added
#   highlight.sector(sn, 
#                    track.index = 1, 
#                    col = color_regions[region], 
#                    text = region, 
#                    niceFacing = TRUE)
# }
# 
...
### Generating bins for clonotype restim ability
```{r Binning_clonotypes}

# set working dir
setwd(working.dir)

# Create output directories
if(!dir.exists("output_tcr/figures/Binning_clonotypes")){
  dir.create("output_tcr/figures/Binning_clonotypes",
             recursive = TRUE)}

output.dir <- "output_tcr/figures/Binning_clonotypes"
setwd(output.dir)


# Take only clonotypes that are shared between US and Stim (these are the most dominant clonotypes anyway)
...
# Get clonotypes found in 
Ex.clonotypes <- Query.clonotypes %>%
  dplyr::filter(seurat_clusters %in% c("Exhausted_1", "Exhausted_2")) %>%
  dplyr::select(CTaa)

Ex.clonotypes <- unique(Ex.clonotypes$CTaa)

Stim.clonotypes <- Query.clonotypes %>%
  dplyr::filter(seurat_clusters %in% c("Stimulated_exhausted", "Stimulated_1")) %>%
  dplyr::select(CTaa)
...
clonotype.list <- list(Exhausted = Ex.clonotypes, 
                       Stimulated = Stim.clonotypes)
...
# Extract vectors
stimulatable.clonotypes <- overlap(clonotype.list)
Terminally.exhausted <- discern(clonotype.list, "Exhausted")
other.stimulatable <- discern(clonotype.list, "Stimulated")
...
# Create seurat object of same subset of cells 
Query.cells <- seurat.tcr@meta.data %>% 
  dplyr::filter(Clonotype_overlap == "Shared" & condition == "Stim") %>%
  dplyr::filter(seurat_clusters %in% c("Exhausted_1", "Exhausted_2", "Stimulated_exhausted", "Stimulated_1")) %>%
  dplyr::distinct(seurat_clusters, CTaa) %>%
  rownames()

Query.seurat <- subset(seurat.tcr, cells = Query.cells) # 1,257 cells 





meta.data.vec <- Query.seurat@meta.data %>%
  dplyr::mutate(Clonotype_stim_response = case_when(CTaa %in% stimulatable.clonotypes ~ "Stimulatable", 
                                              CTaa %in% Terminally.exhausted ~ "Exhausted", 
                                              CTaa %in% other.stimulatable ~ "Other")) %>%
  dplyr::select(Clonotype_stim_response)


Query.seurat <- AddMetaData(Query.seurat, meta.data.vec$Clonotype_stim_response, "Clonotype_stim_response")


UMAPPlot(Query.seurat, 
         pt.size = Query.seurat@meta.data$Total_clonotype_scaled, 
         split.by = "Clonotype_stim_response") + 
  ggtitle("Venn overlap clonotypes")
...
# Plot Shared, and US/Stim unique clonotypes 
x <- clust.clonotypes.seurat@meta.data$Total_clonotype_scaled
...
dev.copy(pdf, "Distribution_of_all_clonotypes_shared_or_unique_to_condition.pdf")
dev.off()
...
# Aim is to group clonotypes by their response after stimulation 

# Use just clonotypes in Stimulated condition 
# Look at all clonotypes present in exhausted clusters 
# distinguish those that are present in stim clusters and those that are unique
# Also, need to consider the frequency across both clusters
...
# Set Idents
Idents(seurat.tcr) <- seurat.tcr@meta.data$CTaa
...
# Get vector of clonotypes found in Stim cells in Exh or Stim clusters


Query.clonotypes <- seurat.tcr@meta.data %>%
  dplyr::filter(condition == "Stim") %>%
  dplyr::filter(seurat_clusters %in% c("Exhausted_1", "Exhausted_2", "Stimulated_exhausted", "Stimulated_1")) %>%
  dplyr::select(CTaa, seurat_clusters)

length(unique(Query.clonotypes$CTaa)) # 852 clonotypes
...
# Extract vectors
shared.clonotypes <- overlap(clonotype.list)
unique.Exhausted <- discern(clonotype.list, "Exhausted")
unique.Stim <- discern(clonotype.list, "Stimulated")

####################
# Add to metadata
####################
meta.data.vec <- seurat.tcr@meta.data %>%
  dplyr::mutate(Clonotype_stim_response = case_when(CTaa %in% shared.clonotypes ~ "Stimulatable", 
                                              CTaa %in% unique.Exhausted ~ "Exhausted", 
                                              CTaa %in% unique.Stim ~ "Other", 
                                              TRUE ~ "Extra")) %>%
  dplyr::select(Clonotype_stim_response)


seurat.tcr <- AddMetaData(seurat.tcr, meta.data.vec$Clonotype_stim_response, "Clonotype_stim_response")




unique(seurat.tcr@meta.data$Clonotype_stim_response)



table(seurat.tcr@meta.data$Clonotype_stim_response, seurat.tcr@meta.data$condition)





unique(seurat.tcr@meta.data$Clonotype_overlap)

Shared.clonotypes <- seurat.tcr@meta.data %>%
  dplyr::filter(Clonotype_overlap == "Shared")


temp <- seurat.tcr@meta.data %>%
  dplyr::filter(condition == "Stim")

outcome <- temp %>%
  dplyr::filter(seurat_clusters %in% c("Exhausted_1", "Exhausted_2", "Stimulated_exhausted", "Stimulated_1")) 
  
exhausted_clonotypes <- outcome %>%
  dplyr::filter(seurat_clusters == "Exhausted_1" | seurat_clusters == "Exhausted_2") %>%
  dplyr::select(CTaa)

  Stimulated_clonotypes <- outcome %>%
  dplyr::filter(seurat_clusters == "Stimulated_exhausted" | seurat_clusters == "Stimulated_1") %>%
  dplyr::select(CTaa)

  
  
 exhausted_clonotypes <- table(exhausted_clonotypes)
    Stimulated_clonotypes <- table(Stimulated_clonotypes)

    length(exhausted_clonotypes)
  
  
  x <- names(exhausted_clonotypes) %in% names(Stimulated_clonotypes)
  sum(x)
   exh.stimable <- names(exhausted_clonotypes)[x]
    exh.nonstimable <- names(exhausted_clonotypes)[!x]
...
# Set Idents
Idents(clust.clonotypes.seurat) <- clust.clonotypes.seurat@meta.data$CTaa

temp.seurat <- subset(clust.clonotypes.seurat, idents = exh.stimable)
temp.seurat <- subset(clust.clonotypes.seurat, idents = exh.nonstimable)
...
Total.clonotype.scale <- temp.seurat@meta.data$Total_clonotype_scaled
