...
rm(combined.contig, contig.list, S1.contig, S2.contig, S3.contig, S4.contig, meta.data)
...
#output <- output[output$from_group != output$to_group, ]
dim(output) # 121 This value makes sense 11 clusters * 11 clusters 

# Extract the clonotypes which are unique to a cluster/condition combination
output <- output[output$from == output$to & output$from_group == output$to_group, ]
# Note there are very few clonotypes which have no overlap within or across conditions 
# out of 257 clonotypes only 6 are unique to a specific clonotype and condition within the entire dataset
colSums(dTest[rowSums(dTest) == 1, ])
nrow(dTest)
...
x <- gsub(paste0(c("^US\\|", "^Stim\\|"), collapse = "|"), "", order.val)
grid.col <- structure(color_species[x], names = order.val)
