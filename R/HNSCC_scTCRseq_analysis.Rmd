...
GLIPH.df <- seurat.tcr@meta.data %>%
  dplyr::select(CTaa, TRBV, TRBJ, group, condition, Clone_conditionwise_n) 
...
# Format dataframe 
GLIPH.df$Subject <- paste0(GLIPH.df$group, ":", GLIPH.df$condition)
head(GLIPH.df)
...
# Format dataframe 
GLIPH.df$Subject <- paste0(GLIPH.df$condition)
head(GLIPH.df)
...
list.files("output_tcr/GLIPH_analysis/", pattern = ".csv")
list.files("output_tcr/GLIPH_analysis/", pattern = ".txt")


# read in data
GLIPH.data <- read.csv("output_tcr/GLIPH_analysis/Sample_condition_cluster.csv")
