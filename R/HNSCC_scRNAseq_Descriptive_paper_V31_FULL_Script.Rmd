...
##################################
# Scale with clonotype frequency
##################################

library("scales")

freq.val <- seurat.tcr@meta.data$Frequency
freq.val <- scales::rescale(freq.val, to = c(0, 10))
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_clusters.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_module.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_clusters_split_condition.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_clusters_split_condition_Nolegend.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_modules_split_condition.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_module_split_condition_Nolegend.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_clusters_grouped_condition.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_clusters_grouped_condition_Nolegend.pdf")
dev.off()
...
# Plot by cluster split by clonetype
UMAPPlot(seurat.tcr, 
         pt.size = freq.val,
         group.by = "seurat_clusters", 
         split.by = "cloneType",
...
dev.copy(pdf, "output/figures/TCRseq/Clonotype_size_UMAP_scaled_clonesize_clusters_split_cloneType.pdf")
dev.off()
...
## Export TCR seurat object {.tabset}
...
### cluster/module specific clonotype function
```{r Get_specific_clonotypes_function}

get.unique.clonotypes <- function(input.seurat, identity.column, ident.id){
  
  # set idents to Modules
  ...
  # get list of clonotypes in selected Module
  Ident.specific.clones <- sort(table(Ident.specific.seurat@meta.data$clonotype_id_A), decreasing = TRUE)
  
  # Find all cells that have these clonotypes
  keep.logic <- input.seurat@meta.data$clonotype_id_A %in% names(Ident.specific.clones)
...
### Module based TCR analysis
```{r Module_based_TCR_vis}


# Create output directories
if(!dir.exists("output/figures/Modules/TCRseq")){
  dir.create("output/figures/Modules/TCRseq",
             recursive = T)}
...
dev.copy(pdf, "output/figures/Modules/TCRseq/UMAP_paired_TCR_per_Module.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/Modules/TCRseq/UMAP_paired_TCR_per_Module_splitby_condition.pdf")
dev.off()
...
dev.copy(pdf, "output/figures/Modules/TCRseq/UMAP_paired_TCR_per_Module_splitby_group.pdf")
dev.off()
...
for(i in 1:length(Module.vect)){
  ...
  # Subset by Module
  Module.specific.seurat <- subset(seurat.tcr, idents = Module.vect[i])
  
  # get list of clonotypes in selected Module
  Module.specific.clones <- sort(table(Module.specific.seurat@meta.data$clonotype_id_A), decreasing = TRUE)
  
  # Find all cells that have these clonotypes
  keep.logic <- seurat.tcr@meta.data$clonotype_id_A %in% names(Module.specific.clones)
  cells.with.module.sp.clones <- rownames(seurat.tcr@meta.data)[keep.logic]
  
  # Subset seurat by these cell IDs
  seurat.module.specific.clones <- subset(seurat.tcr, cells = cells.with.module.sp.clones)
  ...
  # UMAP overall 
  print(UMAPPlot(seurat.module.specific.clones, 
                 group.by = "Module"))
  
  dev.copy(pdf, paste0("output/figures/Modules/TCRseq/UMAP_of_all_clonotypes_found_in_module_", Module.vect[i], ".pdf"))
  dev.off()
  ...
  dev.copy(pdf, paste0("output/figures/Modules/TCRseq/UMAP_of_all_clonotypes_found_in_module_", Module.vect[i], "_splitby_condition.pdf"))
  dev.off()
  
  # Barplot of distribution
  x <- table(seurat.module.specific.clones@meta.data$Module)
  ...
  dev.copy(pdf, paste0("output/figures/TCRseq/Barplot_of_all_clonotypes_found_in_module_", Module.vect[i], ".pdf"))
  dev.off()
  
  
  # calculate the number of unique clones found in the cluster
  unique.clonotypes <- unique(names(Module.specific.clones))
...
dev.copy(pdf, "output/figures/TCRseq/Module_unique_clonotypes.pdf")
dev.off()
