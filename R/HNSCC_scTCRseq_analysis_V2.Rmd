...
for(i in seq_along(color_species)){
  ...
  ####Ploting
  pdf(paste0(dir, "/Circos_inter_condition_US_", names(color_species)[i], ".pdf"), height=10, width=10)
...
temp.df$node_col[index.var] <- color_species[j]
  ...
  link.col <- rep(mycolour("Gray", percent = 80), length(df2$clust_2))
  ...
  ####Ploting
  pdf(paste0(dir, "/Circos_inter_condition_Stim_", names(color_species)[i], ".pdf"), height=10, width=10)
...
####Ploting
pdf(paste0(dir, "/Circos_inter_condition_all_ribbons.pdf"), height=10, width=10)
...
Idents(seurat.tcr) <- seurat.tcr@meta.data$cloneType
index.var <- unique(Idents(seurat.tcr))


for(i in seq_along(index.var)){
  
  temp.seurat <- subset(seurat.tcr, ident = index.var[i])
...
## Investigating Stimulated and Stimulated exhausted clusters {.tabset}

### Stimulated_1 cluster // Incomplete
```{r Stimulated_1_cluster_analysis}

# Purpose
# Investigate the difference between clonotypes in US that move to Stim_1 and those that do not move to stim_1 after stimulation

unique(seurat.tcr@meta.data$condition_clust)



UMAPPlot(clonotype.seurat, 
         group.by = "seurat_clusters",
         split.by = "condition")
...
################################################################################
# Get clones that move from US_Exhausted_1 or 2 into Stim_Stimulated_Exhausted
################################################################################

# Restrict analysis to only clonotypes found in US
clonotype.seurat <- get.clonotypes(seurat.tcr, "condition", "US")

# Now get clonotypes that occur in Stim_Stimulated_Exhausted
clonotype.seurat <- get.clonotypes(clonotype.seurat, "condition_clust", "Stim_Stimulated_exhausted")
...
# Most clones occur in either Exhausted_1 or Exhausted_2 

# Filter data by US_Exhausted_1 or US_Exhausted_2
Idents(clonotype.seurat) <- clonotype.seurat@meta.data$condition_clust

clonotype.seurat <- subset(clonotype.seurat, idents = c("US_Exhausted_1", "US_Exhausted_2"))

# Extract vector of clonotypes that occur in US_Ex1 or 2 and can be found in Stim_stim_ex
Stimed.clones <- unique(clonotype.seurat@meta.data$CTaa)

length(Stimed.clones) # 79 unique clonotypes 



############################################################
# Get all clones in US_Ex1 or 2 clusters
############################################################
...
clonotype.seurat <- subset(seurat.tcr, idents = c("US_Exhausted_1", "US_Exhausted_2"))

Unstimed.clones <- unique(clonotype.seurat@meta.data$CTaa)

length(Unstimed.clones) # 235 unique clonotypes 





# Assign cells either stimable or non-stim 
clonotype.seurat@meta.data$Stim_status <- "Non_stimable" 

logic.vec <- clonotype.seurat@meta.data$CTaa %in% Stimed.clones
clonotype.seurat@meta.data$Stim_status[logic.vec] <- "Stimable"

unique(clonotype.seurat@meta.data$Stim_status)
...
# Reset Idents
Idents(clonotype.seurat) <- clonotype.seurat@meta.data$seurat_clusters


UMAPPlot(clonotype.seurat, 
         group.by = "Stim_status",
         split.by = "condition")
...
DefaultAssay(clonotype.seurat) <- "RNA"

# Find variable features
clonotype.seurat <- FindVariableFeatures(clonotype.seurat,
                                         verbose = TRUE)

# Scale data
clonotype.seurat <- ScaleData(clonotype.seurat, 
                             verbose = TRUE)

# Run PCA
clonotype.seurat <- RunPCA(clonotype.seurat, 
                          npcs = 30, 
                          verbose = TRUE)

# Find Neighbors
clonotype.seurat <- FindNeighbors(clonotype.seurat, 
                                 reduction = "pca", 
                                 k.param = 20,
                                 dims = 1:20)

# Find clusters
clonotype.seurat <- FindClusters(clonotype.seurat, 
                                random.seed = 42,
                                resolution = 0.4) 
...
# Run UMAP
clonotype.seurat <- RunUMAP(clonotype.seurat,
                           reduction = "pca",
                           dims = 1:20,
                           umap.method = "uwot",
...
# No real diff between genetics of responding cytotoxic and non responding 
# is clone size and diversity a difference? 
...
# Set Idents 
Idents(clonotype.seurat) <- clonotype.seurat@meta.data$Stim_status

# Find markers
Stim.markers <- FindAllMarkers(clonotype.seurat,
                                      assay = "RNA",
                                      only.pos = TRUE,
                                      min.diff.pct = 0.1,
                                      return.thresh = 0.05)
...
clonotype.seurat@meta.data$cloneType
...
#### WORK IN PROGRESS BELOW // I am here
...
seurat.cytoIFN <- subset(seurat.tcr, cells = keep.cells)
...
x <- table(seurat.exhausted@meta.data$TRAV, seurat.exhausted@meta.data$Stim_potential)
...
x <- table(seurat.exhausted@meta.data$TRAJ, seurat.exhausted@meta.data$Stim_potential)
...
x <- table(seurat.exhausted@meta.data$TRBV, seurat.exhausted@meta.data$Stim_potential)
...
# TRB-J gene usage
x <- table(seurat.exhausted@meta.data$TRBJ, seurat.exhausted@meta.data$Stim_potential)
...
Nebulosa::plot_density(seurat.exhausted, "CTSW")
