...
genes.id <- c("GZMH", "NKG7", "GZMB", "PRF1", "GZMM", "GZMA", "XCL1", "XCL2", "KLRD1", "CD27", "BACH2", "TIGIT", "ZNF683", "TCF7", "SELL", "CD69", "BATF", "CCR6", "CXCR6", "GPR65", "GPR183", "CDKN1A","NFKBIA", "MYC")
...
# genes of ineterest
goi <- c("TCF7", "BACH2", "BATF", "NKG7", "ZEB2", "HOPX", "ZNF683", "ZBTB16")

for(i in 1:length(goi)){
  
  print(Plot_Density_Custom(Innate.seurat, 
                            features = goi[i],
                            custom_palette = batlow.pal,
                            joint = FALSE, 
                            pt.size = 1, 
  ...
  dev.copy(pdf, paste0(output.Supfig1, "Nebulosa_RNA_", goi[i], ".pdf"))
  dev.off()
...
# plot gene expression of tcr genes 
...
dev.copy(pdf, paste0(output.Supfig1, "/Heatmap_of_TCR_genes_expressed.pdf"))
dev.off()
...
dev.copy(pdf, paste0(output.Supfig1, "/Heatmap_of_TF_diff_expressed.pdf"))
dev.off()
...
activities <- decoupleR::run_wmean(mat = as.matrix(Innate.seurat@assays$RNA@data),
                                   network = network,
                                   .source = "source",
                                   .targe = "target",
                                   .mor = "mor",
...
# General heatmap.
out <- SCpubr::do_TFActivityPlot(sample = Innate.seurat,
...
dev.copy(pdf, paste0(output.Supfig1, "/Heatmap_of_TF_activity_top10.pdf"))
dev.off()
