...
if(quick.load){
  ...
  DefaultAssay(seurat.tcr.filt) <- "RNA"
...
if(!dir.exists("output_tcr/figures/clonalnetworks")){
  dir.create("output_tcr/figures/clonalnetworks",
             recursive = TRUE)}
...
# subset data
Idents(seurat.tcr.filt) <- seurat.tcr.filt@meta.data$condition
US.seurat <- subset(seurat.tcr.filt, idents = "US")
Stim.seurat <- subset(seurat.tcr.filt, idents = "Stim")


# visualise clusters
p1 <- UMAPPlot(seurat.tcr.filt, 
         group.by = "seurat_clusters",
         label = T) + NoLegend() + ggtitle("")

# clusters to plot
clust.ids <- as.character(unique(seurat.tcr.filt@meta.data$seurat_clusters))
